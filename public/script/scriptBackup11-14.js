(function () {
      ...
      // console.log('curves');
        ...
        var mata = new THREE.MeshLambertMaterial({color:0x555555,side:THREE.DoubleSide});
				...
				if (arr.redraw === true) {
                      ...
                      if(arr[i].length>1 && draw){
                        for(var j=arr[i].length-1 ; j<arr[i].length ; j++){
                          ...
                          if(counter+60 >  psys.geometry.attributes.position.array.length)
                            counter=0;
                          else
                            counter+=30;
                          ...
                          for( var k = counter; k < counter+30; k+=3 ) {
                              ...
                              psys.geometry.attributes.position.array[k]   = arr[i][j].x+Math.random()*.1;
                              psys.geometry.attributes.position.array[k+1] = arr[i][j].y+Math.random()*.1;
                              psys.geometry.attributes.position.array[k+2] = arr[i][j].z+Math.random()*.1;

                              psys.geometry.attributes.size.array[p]=.02;
    ...
    function setupParticles(amount,size,texture){



noiseShader = ["vec3 mod289(vec3 x)",
                "{",
...
//                 mvPosition.y+=(ns+(ns*ns2)*offer)*size*10.;\
//                 mvPosition.x+=(mvPosition.z*.0051)*nsy+(nsy*nsy2)*offer;\
        //(10.+size*2.)*pSize*min(xyoff*.1,1.)*.6 + max((((-size+3.)*5.)),0.);\
        ...
        lightShader.uniforms = {
            ...
            time:      { type: 'f', value: 0 },
            pSize:     { type: 'f', value: 1 },
            offz:      { type: 'f', value: 5 },
            boing:     { type: 'f', value: 1 },
            swirl:     { type: 'f', value: 0 },
            offer:     { type: 'f', value: 5 },
            color:     { type: "c", value: new THREE.Color( 0xffffff ) },
            texture:   { type: "t", value: THREE.ImageUtils.loadTexture( "img/cat4.png" ) }

        };

        noiseShaderConcat = noiseShader+lightShader.vertexShader;
        ...
        for( var v = 0; v < particles; v++ ) {
            ...
            values_size[ v ] = .02;//Math.random()*.1;
        ...
        psys = new THREE.PointCloud( geo, shaderMaterial );
        ...
        // psys.geometry.attributes.size.needsUpdate = true;
        // psys.geometry.attributes.position.needsUpdate = true;
        // psys.material.uniforms.time.value = time;
        // psys.material.uniforms.offz.value = drAvg+15;
        // psys.material.uniforms.offer.value = 5+(bassTrack[DR]*.1);
        // psys.material.uniforms.pSize.value = zed+Math.min((drumTrack[DR+1])*.002,1);

        // psys.material.uniforms.boing.value = (drAvg-1)*10;//*mouseX*30;

        // if(var2)
        //     psys.material.uniforms.swirl.value = swirlAvg.avg(1);
        // else
        //     psys.material.uniforms.swirl.value = swirlAvg.avg(0);
